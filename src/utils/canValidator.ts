...
export function validateCommand(
  ...
  if (command.state_can_id) {
    const stateCanResult = validateCanId(command.state_can_id);
    if (!stateCanResult.isValid) {
      ...
      issues.push({
        type: 'info',
        field: 'state_can_id',
        message: `Extended 29-bit State CAN ID detected (${command.state_can_id}).`,
        commandId: command.id
      });
    }
  } else {
    // If it's a can_tx or can_state, state_can_id is usually expected
    if (command.type === 'can_tx' || command.type === 'can_state') {
      issues.push({
        type: 'warning',
        field: 'state_can_id',
        message: `Command type is '${command.type}', but no State CAN ID is specified.`,
        commandId: command.id
      });
    }
  }
  ...
  if (command.action_can_id) {
    const actResult = validateCanId(command.action_can_id);
    if (!actResult.isValid) {
      ...
      issues.push({
        type: 'info',
        field: 'action_can_id',
        message: `Extended 29-bit Action CAN ID detected (${command.action_can_id}).`,
        commandId: command.id
      });
    }
  }
  ...
  // Bus validation
  if (command.bus !== undefined && (typeof command.bus !== 'number' || command.bus < 0)) {
    issues.push({
      type: 'error',
      field: 'bus',
  ...
  if (command.from_payload !== undefined) {
    const res = validatePayloadString(command.from_payload);
    if (!res.isValid) {
  ...
  if (command.to_payload !== undefined) {
    const res = validatePayloadString(command.to_payload);
    if (!res.isValid) {
      issues.push({ type: 'error', field: 'to_payload', message: res.error!, commandId: command.id });
    }
  }

  if (command.match_payload !== undefined) {
    const res = validatePayloadString(command.match_payload);
    if (!res.isValid) {
      issues.push({ type: 'error', field: 'match_payload', message: res.error!, commandId: command.id });
    }
  }
  ...
  if (command.options && command.options.length > 0) {
    command.options.forEach((opt, idx) => {
      ...
      if (opt.payload) {
        const optRes = validatePayloadString(opt.payload);
        if (!optRes.isValid) {
          issues.push({
            type: 'error',
      ...
      if (opt.from_payload) {
        const optRes = validatePayloadString(opt.from_payload);
        if (!optRes.isValid) {
          issues.push({
            type: 'error',
      ...
      if (opt.to_payload) {
        const optRes = validatePayloadString(opt.to_payload);
        if (!optRes.isValid) {
          issues.push({
            type: 'error',
      ...
      if (opt.match_payload) {
        const optRes = validatePayloadString(opt.match_payload);
        if (!optRes.isValid) {
          issues.push({
            type: 'error',
            field: `options[${idx}].match_payload`,
            message: `Option #${idx + 1} match_payload error: ${optRes.error}`,
            commandId: command.id
          });
        }
      }
      ...
      if (opt.steps && opt.steps.length > 0) {
        opt.steps.forEach((step, sIdx) => {
          if (!step.payload) {
            ...
            const stepRes = validatePayloadString(step.payload);
            if (!stepRes.isValid) {
              issues.push({
                type: 'error',
                field: `options[${idx}].steps[${sIdx}].payload`,
  ...
  if (command.steps && command.steps.length > 0) {
    command.steps.forEach((step, idx) => {
      if (!step.payload || step.payload.trim() === '') {
        issues.push({
          type: 'error',
