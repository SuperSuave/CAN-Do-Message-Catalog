...
interface CommandFilterProps {
  ...
  totalCount: number;
  activeMainTab?: 'catalog' | 'vehicles';
  onChangeMainTab?: (tab: 'catalog' | 'vehicles') => void;
}
...
export const CommandFilter: React.FC<CommandFilterProps> = ({
  ...
  return (
    <div className="space-y-3.5">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 min-w-0">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <input
            ...
            className="w-full pl-10 pr-9 py-2.5 rounded-full bg-[var(--input-bg)] border border-[var(--border-color)] text-xs text-[var(--text-heading)] placeholder-slate-500 focus:outline-none focus:border-[var(--md-sys-color-primary)] shadow-sm transition"
          />
          {search && (
            <button
        ...
        <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto">
          ...
          <button
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-3.5 sm:px-4 py-2 rounded-full bg-[var(--md-sys-color-primary)] hover:opacity-90 text-white font-semibold text-xs shadow-sm transition whitespace-nowrap"
          >
      ...
      {/* Role Switcher & Command Category Row */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pt-0.5 min-w-0">
        <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 min-w-0">
          ...
          {onChangeMainTab && (
            <div className="ha-segmented-group text-xs shrink-0">
              <button
                ...
                className={`px-3 sm:px-3.5 py-1.5 rounded-full font-bold transition flex items-center gap-1.5 ${
                  activeMainTab === 'catalog'
                    ? 'active !text-white'
          ...
          {/* Role Tabs */}
          <div className="flex flex-wrap items-center gap-1 p-1 rounded-2xl sm:rounded-full bg-[var(--card-bg)] border border-[var(--border-color)] text-xs">
            <button
              type="button"
        ...
        {/* Category, View Mode & General Reset */}
        <div className="flex flex-wrap items-center justify-between sm:justify-start gap-2 text-xs w-full md:w-auto min-w-0">
          {/* Category dropdown */}
          <select
            ...
            className="flex-1 sm:flex-initial max-w-full px-3 py-1.5 rounded-[8px] bg-[var(--input-bg)] border border-[var(--border-color)] text-[var(--text-heading)] focus:outline-none focus:border-[var(--md-sys-color-primary)] text-xs min-w-0"
          >
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span className="text-[var(--text-muted)] font-mono text-[11px]">
              {filteredCount} / {totalCount} cmds
            </span>
          </div>
        </div>
      </div>

      {/* Subcategory / Subsystem Chips Bar */}
      {availableSubcategories.length > 0 && (
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1 text-xs min-w-0">
          <span className="text-slate-400 text-[11px] font-medium flex items-center gap-1 shrink-0 mr-0.5">
            <Layers className="w-3.5 h-3.5 text-cyan-400" />
            <span className="hidden sm:inline">Subsystem:</span>
          </span>
          <button
            type="button"
            onClick={() => onSubcategoryChange?.('all')}
            className={`px-2.5 py-1 rounded-full text-[11px] font-medium shrink-0 transition ${
              selectedSubcategory === 'all'
          ...
          {availableSubcategories.map(subcat => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-[11px] font-medium shrink-0 transition flex items-center gap-1.5 ${
                  isSelected
      ...
      {/* Rich Vehicle & Hardware Equipment Filtering Bar (Matches Vehicles Tab) */}
      <div className="rounded-xl border border-[var(--border-color)] bg-[var(--card-bg)]/80 p-2.5 sm:p-3 space-y-2.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 text-xs min-w-0">
          ...
          <div className="relative min-w-0">
            <select
              ...
              className="w-full max-w-full px-3 py-2 rounded-[8px] bg-[var(--input-bg)] border border-[var(--border-color)] text-[var(--text-heading)] focus:outline-none focus:border-[var(--md-sys-color-primary)] transition min-w-0"
            >
          ...
          <div className="relative min-w-0">
            <select
              ...
              className={`w-full max-w-full px-3 py-2 rounded-[8px] bg-[var(--input-bg)] border focus:outline-none transition min-w-0 ${
                selectedVehicleId !== 'all'
        ...
        {/* 5. Active Selected Vehicle Banner (When a specific vehicle trim is selected) */}
        {selectedVehicle && (
